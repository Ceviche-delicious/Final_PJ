...
def setup_logging(expname, basedir):
    from datetime import datetime
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    logdir = os.path.join(basedir, expname, 'training_logs')
    os.makedirs(logdir, exist_ok=True)
    log_filename = os.path.join(logdir, f'training_{current_time}.log')
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_print==0:
            with torch.no_grad():
                img_i = np.random.choice(i_test)          # 随机抽一张测试图
                target_test = images[img_i]
                if not use_batching:                      # 非 batching 情况下仍在 CPU，需要搬到 GPU
                    target_test = torch.Tensor(target_test).to(device)
                pose_test = poses[img_i, :3, :4]

                rgb_test, _, _, _ = render(
                    H, W, K, chunk=args.chunk, c2w=pose_test,
                    **render_kwargs_test
                )

                test_img_loss = img2mse(rgb_test, target_test)
                test_psnr     = mse2psnr(test_img_loss)
            # tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}")
            tqdm.write(
                f"[TRAIN] Iter: {i}  "
                f"Loss: {loss.item():.6f}  PSNR: {psnr.item():.2f} | "
                f"[TEST] Loss: {test_img_loss.item():.6f}  PSNR: {test_psnr.item():.2f}"
            )
            logging.info(f"[TRAIN] Iter: {i} Loss: {loss.item()} PSNR: {psnr.item()}")
            logging.info(f"[TEST] Iter: {i} Loss: {test_img_loss.item()} PSNR: {test_psnr.item()}")
            writer.add_scalar('train_loss', loss.item(), i)
            writer.add_scalar('train_PSNR', psnr.item(), i)
            writer.add_scalar('test_loss', test_img_loss.item(), i)
            writer.add_scalar('test_PSNR', test_psnr.item(), i)
        """
